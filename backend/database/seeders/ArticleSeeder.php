...
class ArticleSeeder extends Seeder
{
    ...
    {
        // ------------------------------------------------------------------
        // 1. Clean slate — truncate in dependency order so FK constraints
        //    are satisfied on both MySQL and PostgreSQL.
        // ------------------------------------------------------------------
        $this->command->info('ArticleSeeder: truncating existing data…');
        ...
        $this->command->info('ArticleSeeder: tables cleared.');

        // ------------------------------------------------------------------
        // 2. Wrap all inserts in a transaction so a partial failure is
        //    visible rather than leaving the database in a broken state.
        // ------------------------------------------------------------------
        DB::transaction(function () {
            ...
            $tags = collect($tagData)->map(function (array $data) {
                $tag = Tag::create($data);
                $this->command->info("  [tag]     created → {$tag->slug} (id={$tag->id})");
                return $tag;
            });
...
MD,
            ...
            foreach ($articles as $data) {
                ...
                $this->command->info("  [article] created → \"{$article->title}\" (id={$article->id})");
                ...
                $this->command->info("            tags attached → " . implode(', ', $tagSlugs));
            }
        });
        ...
        $this->command->info("ArticleSeeder: done — {$articleCount} articles, {$tagCount} tags in database.");
    }
}
